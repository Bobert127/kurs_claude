-- =============================================================================
-- HSBC - weryfikacja normy tygodniowej
-- Wersja 2 - poprawa wydajnosci rozwiazania
--
-- Zmiany wzgledem wersji 1:
--   * kal_base: dodany filtr "czas_nom IS NOT NULL" - eliminuje puste tygodnie
--     juz na wejsciu, dzieki czemu czas_nom_agg produkuje tylko tygodnie
--     z faktycznymi danymi.
--   * czas_nom_agg dolaczany przez INNER JOIN zamiast LEFT JOIN - usuwa
--     wiersze bez czasu nominalnego (kolumna "CZAS_NOM z kalendarza [h]" = NULL).
--   * Hint LEADING(p o t cn) USE_HASH(cn) - wymusza start od malych
--     zmaterializowanych CTE i hash join dla czas_nom_agg.
-- =============================================================================
WITH
        kalendarze AS (
            SELECT /*+ MATERIALIZE */
        ...
        prac_hr AS (
            SELECT /*+ MATERIALIZE */
                   ...
                   LEAST(NVL(p.data_rozw, DATE '2026-07-05'), DATE '2026-07-05') AS data_ref, akt_dane.j_org(p.prac_id, LEAST(NVL(p.data_rozw, DATE '2026-07-05'), DATE '2026-07-05')) AS jednostka_org,
                   akt_dane.mpk(p.prac_id, LEAST(NVL(p.data_rozw, DATE '2026-07-05'), DATE '2026-07-05')) AS mpk,
                   akt_dane.stanowisko(p.prac_id, LEAST(NVL(p.data_rozw, DATE '2026-07-05'), DATE '2026-07-05')) AS stanowisko
            FROM t_prac p
